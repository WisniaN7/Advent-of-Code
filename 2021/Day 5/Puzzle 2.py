...
with open(r'2021\Day 5\input.txt') as f:
    for line in f:
        ...
        if delta[0] == 0 and delta[1] == 0:
            ...
            increment = (1 if delta[0] > 0 else -1, 1 if delta[1] > 0 else -1)
            
            for x, y in zip(range(int(x1), int(x2) + increment[0], increment[0]), range(int(y1), int(y2) + increment[1], increment[1])):
                diagram[y][x] += 1
